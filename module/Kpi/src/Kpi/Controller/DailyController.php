...
class DailyController extends AbstractActionController
{
    ...
    {
        $json_file = 'C:/nivea_kpi/json/work/kpi_daily.json';
        $content = (string) S::create(file_get_contents($json_file))->collapseWhitespace();
        $json = json_decode($content, true);

        if (!is_null($json)) {
            file_put_contents('C:/nivea_kpi/json/final/kpi_daily.json', json_encode($json));
        }

        $json = json_decode(file_get_contents('C:/nivea_kpi/json/final/kpi_daily.json'), true);
